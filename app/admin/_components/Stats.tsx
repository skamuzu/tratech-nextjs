...
export default function Stats() {
  return (
    <div className="grid grid-cols-3 w-full container justify-around gap-4 py-8 mx-auto">
      {List.map((item) => {
        return (
          <Card key={item.title} className="w-full flex flex-col">
            ...
            <CardContent className="text-muted-foreground text-lg">{item.title}</CardContent>
          </Card>
        );
      })}
    </div>
